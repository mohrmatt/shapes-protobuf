...
data type ShapeType from shape_type_key.idl.
This example builds two applications, named shape_type_key_publisher and
shape_type_key_subscriber.
...
==============
This example is generated with the following options:
> rtiddsgen -example <arch> -exampleTemplate minimal <idl>.idl
This example generates the minimal code to publish and subscribe to a topic.
The publisher code is similar to the simple example, but the subscriber
code is simplified using a Sample Processor to process the received data.
The Sample Processor is a callback that is triggered when data is received.
...
> make -f makefile_shape_type_key_x64Linux4gcc8.5.0
This command will build a release executable.
 
To build a debug version instead:
> make -f makefile_shape_type_key_x64Linux4gcc8.5.0 DEBUG=1

To Modify the Data:
===================
To modify the data being sent, edit the shape_type_key_publisher.cxx
file where it says:
...
Run $NDDSHOME/resource/scripts/rtisetenv_x64Linux4gcc8.5.0.bash
...
> objs/x64Linux4gcc8.5.0/shape_type_key_publisher -d <domain_id> -s <sample_count>
> objs/x64Linux4gcc8.5.0/shape_type_key_subscriber -d <domain_id> -s <sample_count>
